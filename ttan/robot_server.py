	...
	def handle(self):
		...
		while True:
			...
			while robot:
				...
				if buf == str(MSG_JOINT_STATES) or buf == "status":
				...
				elif command == "speedj": #IMPLEMENTED by NS
					pose = list()
					a = 1.2
					t_min = 0.3
					wp = 999
					try:
						if buf.find('(') == -1 or buf.find(']') == -1 or buf.find(']') == -1 or buf.find(')') == -1:
							self.request.send("Poorly formatted SPEEDJ command (Could not find expected parenthesis or brackets)\n")
							continue
						for ch in buf.split('[')[1].split(']')[0].split(','):
							pose.append(float(ch))
						if len(pose) != 6:
							self.request.send("Poorly formatted SPEEDJ command (Supply 6 values for the pose speed)\n")		
							continue			
						params = buf.split(']')[1][:-1].split(',')
						if len(params) >= 2: #first is the (empty) string between ] and ,
							a = float(params[1])
						...
						if len(params) >= 4:
							wp = float(params[3])
						print "got a SPEEDJ command to waypoint " +str(wp)+ " at pose " + str(pose) + " with params: " + str(a) + ', ' + str(t_min)
						params = [MSG_SPEEDJ, wp] + \
								[MULT_jointstate * q for q in pose] + \
								[MULT_jointstate * a, MULT_jointstate * t_min]
						buf = struct.pack("!%ii" % len(params), *params)
						robot.send_msg(buf)
					except:
						self.request.send("Poorly formatted SPEEDJ command (Unknown error)\n")		
						continue
