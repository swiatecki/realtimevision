def driverProg():
		...
		while True:
			...
			if state == QUEUE_RUNNING:
				if id == MSG_SPEEDL:
					...
					send_out("speedj started")
					speedj(q, params[0], params[1])
					send_waypoint_finished(waypoint_id)
					send_out("speedj finished")
				end
			else:
				#send_out("Idle")
				sync()
	...
	while True:
		...
		if ll[0] == 0:
			...
			if mtype == MSG_QUIT:
			...
			elif mtype == MSG_SPEEDJ:
				send_out("Received speedj")
				params_mult = socket_read_binary_integer(1+6+2)
				if params_mult[0] == 0:
					send_out("Received no parameters for speedj message")
				end
				waypoint_id = params_mult[1]
				q = [params_mult[2] / MULT_jointstate,
					params_mult[3] / MULT_jointstate,
				...
				a = params_mult[8] / MULT_jointstate
				t_min = params_mult[9] / MULT_jointstate
				set_queue(MSG_SPEEDJ, q, [a, t_min, 0, 0])
				#speedj(q,a,t)
